...
const ImportAccounts = (props) => {
    const { isSeedPhrase, cleanedSeed, allwallets, loading, accountSelection, setAccountSelection, accountsView, setAccountsView, onPressContinue } = useImportAccounts(props)
    return (
        <MainContainer>
            <Spacer customHeight={hp(6)} />
            <View style={styles.mainView}>
                <SeedPhraseCustomHeader leftImage={Images.goBackArrow} centerImage={Images.slideLine2} rightImage={'none'}
                    onPressLeftImage={() => {
                        if (accountsView) {
            ...
            <View style={{ paddingBottom: hp(4) }}>
                ...
                {accountsView == false ?
                    <>
                        <CustomButton
                            title='View Accounts'
                            btnSyles={{ backgroundColor: colors.gray21 }} titleStyles={{ color: colors.gray22 }}
                            onPressBtn={() => setAccountsView(true)} />
                        <Spacer customHeight={hp(2)} />
                    </> : null}
                <CustomButton
                    title='Continue'
